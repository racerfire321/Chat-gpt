...
function ChatInput  ({chatId}: Props) {
    ...
    const sendMessage = async(e: FormEvent<HTMLFormElement>) =>{
      ...
      await addDoc(collection(db, 'user', session?.user?.email!,'chats',chatId,'messages'),message)
  ...
  return (
    <div className='bg-gray-700/50 text-gray-400 rounded-lg text-sm'>
        <form onSubmit={sendMessage}className="p-5 space-x-5 flex">
            ...
            <button disabled={!prompt || !session} type='submit' className='bg-[#11A37F] hover:opacity-50 text-white font-bold px-4 py-2 rounded disabled:bg-gray-300
